{17/7/22 - corrected bug to allow [M] in no-keyword responses}
...
var fixedset: integer;
begin
 seqdefault:=seq;
 for fixedset:=1 to lastfixedset do
  fixedsetarray[fixedset].sequential:=seqdefault;
 with ElizaForm do
  case seq of
...
begin
 if EditForm.EditorMemo.Modified then
  if not(query('This operation will lose the editing changes that you have made. '
   ...
   exit;
 copyscript(progname);
 if progname='Weizenbaum.txt' then
  copyscript('WeizDoctor.doc'); {special case for Weizenbaum dialogue}
