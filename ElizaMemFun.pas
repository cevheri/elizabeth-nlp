    ...
    begin
      ...
      begin {selection is necessary}
       ...
       repeat
        if sequential then
         ...
         end; {RESPNUM has now been selected - not yet tested for duplication with lastresponse}
        fixedresps[respnum].tried:=true;
        inc(resptried);
        if (fixedresps[respnum].index=lastresponse) and (resptried<numresponses) then
         begin
          respok:=false;    {if last response has been selected, prohibit it for now}
          resplast:=respnum {but save it as RESPLAST to try again if all others fail}
         end
        else
         begin
     ...
     if not(respok) then
      begin
       ...
       tempactive:=fixedresps[respnum].msgstring; {added 17/7/22 - so [M] can be used etc.}
       respok:=(condok(fixedcodes[fixedset,1]+' '+fixedresps[respnum].index,
                  fixedresps[respnum].condstr))
               and (memiocheck(1,tempactive,dontapply,donttrace))
      end;
     if respok then
      begin
       ...
       memiocheck(0,result,doapply,dotrace); {ADDED 4/11/2022 so as to apply to Welcome messages}
